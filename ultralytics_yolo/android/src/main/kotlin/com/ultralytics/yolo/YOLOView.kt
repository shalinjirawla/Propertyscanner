            ...
            when (task) {
                ...
                YOLOTask.DETECT -> {
                    ...
                    for (box in result.boxes) {
                        // Filter out "object" class
                        if (box.cls.equals("object", ignoreCase = true)) {
                            Log.d(TAG, "Skipping 'object' class detection")
                            continue
                        }

                        // === USE NORMALIZED COORDINATES ===
                        // box.xywhn contains values from 0-1, scale to view size
                        var left   = box.xywhn.left   * vw
                        var top    = box.xywhn.top    * vh
                        var right  = box.xywhn.right  * vw
                        var bottom = box.xywhn.bottom * vh

                        Log.d(TAG, "Box ${box.cls}: norm=(${box.xywhn.left},${box.xywhn.top},${box.xywhn.right},${box.xywhn.bottom})")
                        Log.d(TAG, "Box ${box.cls}: view=($left,$top,$right,$bottom)")
                        ...
                        if (left < 0 || top < 0 || right > vw || bottom > vh || right <= left || bottom <= top) {
                            Log.w(TAG, "Invalid box coordinates: L=$left T=$top R=$right B=$bottom")
                            continue
                        }
                        ...
                        paint.alpha = 255  // Full opacity for box
                        ...
                        Log.d(TAG, "✓ Drew box for ${box.cls} at ($left,$top,$right,$bottom)")
                ...
                YOLOTask.SEGMENT -> {
                    ...
                    for (box in result.boxes) {
                        ...
                        // Draw bounding box
                        // Use normalized coordinates (0-1) and scale to view size
                        var left   = box.xywhn.left   * vw
                        var top    = box.xywhn.top    * vh
                        var right  = box.xywhn.right  * vw
                        var bottom = box.xywhn.bottom * vh

                        Log.d(TAG, "Box ${box.cls}: L=$left T=$top R=$right B=$bottom")

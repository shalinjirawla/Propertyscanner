...
public class YOLOView: UIView, VideoCaptureDelegate {
   ...
   private var allowedClasses: Set<Int> = [0, 1, 3]
  ...
  func onPredict(result: YOLOResult) {
    print("DetecteddFromYOLOView-:---- \(result)")
 ...
 func showBoxes(predictions: YOLOResult) {
   ...
   // Filter to ONLY show allowed classes (0, 1, 3)
   let filteredBoxes = predictions.boxes.filter { box in
     return allowedClasses.contains(box.index)
   }

   resultCount = filteredBoxes.count
   ...
   if UIDevice.current.orientation == .portrait {
     ...
     for i in 0..<boundingBoxViews.count {
       if i < resultCount && i < 50 {
         ...
         switch task {
         case .detect:
           let prediction = filteredBoxes[i]  // Use filtered boxes
           rect = CGRect(
     ...
     for i in 0..<boundingBoxViews.count {
       if i < resultCount && i < 50 {
         ...
         let colorIndex = filteredBoxes[i].index % ultralyticsColors.count
         boxColor = ultralyticsColors[colorIndex]
         label = String(format: "%@ %.1f", bestClass, confidence * 100)
         alpha = CGFloat((confidence - 0.2) / (1.0 - 0.2) * 0.9)
 ...
 // OPTIONAL: Add a public method to configure allowed classes dynamically
 public func setAllowedClasses(_ classIndices: Set<Int>) {
   self.allowedClasses = classIndices
 }
